  ...
  Future<void> _calculateReports() async {
    try {
      ...
      for (var inv in invoices) {
        if (selectedYear != "All" &&
            inv.invoiceDate?.year.toString() != selectedYear) {
          continue;
        }
        ...
        customerMap[inv.customerName] =
            (customerMap[inv.customerName] ?? 0) + inv.grandTotal;
        ...
        for (var item in inv.items) {
          ...
          productMap[item.itemName] =
              (productMap[item.itemName] ?? 0) + item.qty;
        }
      }

      var sortedCustomers = customerMap.entries.toList()
            ...
            children: [
              ...
              KTable(
                ...
                columns: [
                  ...
                  KTableColumn(
                    label: Label("SPEND", weight: 700).regular,
                    numeric: true,
                  ),
                  KTableColumn(
                    label: Label("SHARE", weight: 700).regular,
                    numeric: true,
                  ),
                ],
                rows: topCustomers.map((e) {
                  final percentage =
                      (e.value / (totalRevenue == 0 ? 1 : totalRevenue)) * 100;
                  return KTableRow(
                    cells: [
            ...
            children: [
              ...
              KTable(
                ...
                columns: [
                  ...
                  KTableColumn(
                    label: Label("UNITS", weight: 700).regular,
                    numeric: true,
                  ),
  ...
  Widget _buildRevenueOverview() {
    return GridView.count(
      ...
      children: [
        _statCard(
          "Total Revenue",
          kCurrencyFormat(totalRevenue),
          LucideIcons.trendingUp,
          Colors.green,
        ),
        _statCard(
          "Metal Sales",
          kCurrencyFormat(metalRevenue),
          LucideIcons.gem,
          Colors.amber,
        ),
        _statCard(
          "Service Income",
          kCurrencyFormat(serviceRevenue),
          LucideIcons.settings2,
          Colors.blue,
        ),
        _statCard(
          "Tax Collected",
          kCurrencyFormat(totalGst),
      ...
      child: topCustomers.isEmpty
          ...
          : Column(
              children: topCustomers.map((entry) {
                final percentage =
                    (entry.value / (totalRevenue == 0 ? 1 : totalRevenue)) *
                    100;
                return _listItem(
                  title: entry.key,
              ...
              : SizedBox(
                  ...
                  child: Row(
                    ...
                    children: monthlySales.entries.map((entry) {
                      final maxVal = monthlySales.values.reduce(
                        (a, b) => a > b ? a : b,
                      );
                            ...
                            decoration: BoxDecoration(
                              gradient: LinearGradient(
                                ...
                                colors: [
                                  ...
                                  kColor(context).primary.withAlpha(100),
                                ],
                              ),
